...
LoopClosing::LoopClosing(Map *pMap, KeyFrameDatabase *pDB, ORBVocabulary *pVoc, const bool bFixScale,shared_ptr<PointCloudMapping> pPointCloud):
    ...
    mbStopGBA(false), mpThreadGBA(NULL), mbFixScale(bFixScale), mnFullBAIdx(0), mpPointCloudMapping(pPointCloud)
{
            ...
            if(DetectLoop())
            {
               ...
               if(ComputeSim3())
               {
                   // Perform loop fusion and pose graph optimization
                    ...
                    // // INFO:
                    // printf("检测到回环检测！\n");
                    // mpPointCloudMapping->bIsLoop = true ;
               }
            }
        }       
...
void LoopClosing::RunGlobalBundleAdjustment(unsigned long nLoopKF)
{
    ...
    // We need to propagate the correction through the spanning tree
    {
        ...
        if(!mbStopGBA)
        {
            ...
            cout << "全局ba优化完成！" << endl;
            mpPointCloudMapping->bIsLoop = true ;
            // mpPointCloudMapping->setIsLoop(true);
